...
class Pool extends EventEmitter {
  ...
  jobTimeout() {
    ...
    setTimeout(async() => {
      if (now >= this.lastActive + Pool.MAX_TIME) {
        this.current = null;
        this.lastActive = now;
  ...
  getPayoutList(addr, limit, offset) {
    return this.stats.getUserList(addr, limit, offset);
  }
...
Pool.MAX_JOBS = 6;
Pool.BAN_TIME = 10 * 60;
// create new job if block not found in 1hour
// FIX THIS: Should reset if nonce exhaust
Pool.MAX_TIME = 1 * 60 * 60;
